...
p2 = ggplot(df_entropy %>% filter(solution_category=="complex"),aes(y=exp_entropy,x=as.factor(number_solutions_used)))+
  ...
  labs(x="Known complex variants",y="",size="Total\nproductions")+
  theme_classic()
g1 = ggarrange(p1,p2, labels = "AUTO", nrow = 1, common.legend = T,legend="right")
g1
ggsave(g1, file="../output/Fig4.pdf",width=11.8,height=6,units="cm",scale=2)
